  ...
  return (
    <div className="relative overflow-hidden rounded-[24px] border border-blue-100 bg-white p-6 shadow-[0_12px_40px_rgba(23,44,113,0.08)]">
      <div className="flex flex-wrap items-start justify-between gap-3 mb-5">
        <div>
          <p className="text-[0.65rem] font-black uppercase tracking-wider text-slate-400">Active Loan ID</p>
          <p className="font-mono text-xs text-brand-muted mt-0.5">{loan.applicationUuid}</p>
        </div>
        <span
          className={cn(
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {/* Outstanding Payment - visually prominent */}
        <div className="relative overflow-hidden rounded-[20px] bg-gradient-to-br from-[#12244f] to-[#0a1628] p-5 text-white shadow-[0_12px_24px_rgba(18,36,79,0.18)]">
          ...
          <p className="mt-2 text-[0.72rem] text-slate-300 font-medium">Includes Principal + Interest + Fees & GST</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <StatBlock
            label="Bank Account"
            value={loan.bankDisplay || '—'}
          />
        </div>
      </div>

      {loan.disbursedAt && (
        <div className="mt-5 pt-4 border-t border-slate-100 flex flex-wrap items-center justify-between text-xs text-brand-muted">
    ...
    <div className="rounded-[20px] border border-[rgba(18,36,79,0.1)] bg-white p-5 shadow-sm transition-all hover:shadow-[0_8px_24px_rgba(23,44,113,0.06)]">
      ...
      <dl className="grid gap-3 sm:grid-cols-2 text-sm">
        ...
        <div>
          ...
          <dd className="font-semibold text-brand-navy">{loan.tenureDays != null ? `${loan.tenureDays} days` : '—'}</dd>
        </div>
        <div>
        ...
        {loan.disbursedAt && (
          <div className="sm:col-span-2 pt-2 border-t border-slate-50 flex justify-between">
            <dt className="text-[0.7rem] font-bold uppercase tracking-wider text-slate-400">Disbursed At</dt>
            <dd className="font-semibold text-brand-navy text-xs">
              {new Date(loan.disbursedAt).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' })}
            </dd>
          </div>
        )}
        {loan.bankDisplay && (
          <div className="sm:col-span-2 flex justify-between">
            <dt className="text-[0.7rem] font-bold uppercase tracking-wider text-slate-400">Bank Destination</dt>
            <dd className="font-semibold text-brand-navy text-xs">{loan.bankDisplay}</dd>
          </div>
        )}
      </dl>
    </div>
...
export function MyAccountSection() {
  ...
  const resumeHref = useMemo(() => {
    return session && isCustomerPortalSignedIn(session)
      ? getCustomerJourneyResumePath(session)
      : '/apply-for-loan';
  }, [session]);

  const journeySteps = useMemo(() => {
    return buildJourneySteps(session);
  }, [session]);

  const showIncompleteJourney = useMemo(() => {
    if (!session || !session.authenticated || !session.lead) return false;
    // Journey is incomplete if references are not completed, and not rejected
    return !session.journey.referencesCompleted && !isLeadRejectedAndLocked(session.lead);
  }, [session]);
  ...
  return (
    <div className="grid gap-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-[0.7rem] font-extrabold uppercase tracking-[0.2em] text-slate-400">My Account</p>
          <h1 className="mt-1 text-[clamp(1.75rem,4vw,2.25rem)] font-extrabold tracking-tight text-brand-navy">
            {greetingName ? `Welcome back, ${greetingName}` : 'My Account'}
          </h1>
          <p className="mt-2 max-w-xl text-brand-muted leading-relaxed">
            Manage your applications, active loans, and repayments in one place.
          </p>
        </div>
      </div>

      {/* Underline Style Tab Strip */}
      <div className="flex border-b border-[rgba(18,36,79,0.08)] mb-2">
        <button
          onClick={() => setActiveTab('current')}
          className={cn(
            'pb-4 px-4 font-extrabold text-[0.95rem] transition-all relative outline-none',
            activeTab === 'current' ? 'text-brand-blue' : 'text-slate-400 hover:text-brand-navy'
          )}
        >
          Active & In-Progress
          {activeTab === 'current' && (
        ...
        <button
          ...
          className={cn(
            'pb-4 px-4 font-extrabold text-[0.95rem] transition-all relative outline-none',
            activeTab === 'history' ? 'text-brand-blue' : 'text-slate-400 hover:text-brand-navy'
          )}
        >
          Previous Loans
          {activeTab === 'history' && (
      ...
      {activeTab === 'current' ? (
        <div className="grid gap-6 animate-fade-in-up">
          ...
          {/* 2. Active disbursed loan and Amount to Repay */}
          {dash.activeLoans.length > 0 ? (
            <section className="grid gap-5">
              <h2 className="text-lg font-extrabold text-brand-navy">Running Loan</h2>
            ...
            // Only show active empty state if there is also NO incomplete journey
            !showIncompleteJourney && (
              <EmptyStateCard
                icon={<WalletIcon />}
                title="No active loan yet"
                description="Once your application is approved and disbursed, your active loan and repayment details will appear here."
                action={{ label: 'Apply for a loan', href: '/apply-for-loan' }}
              />
            )
          )}

          {/* 3. Repayment Schedule (Only shown if active loan exists) */}
          {dash.activeLoans.length > 0 && dash.repaymentSchedule.length > 0 && (
            <section className="grid gap-4">
              <h2 className="text-lg font-extrabold text-brand-navy">Repayment Details</h2>
              <div className="overflow-hidden rounded-[20px] border border-[rgba(18,36,79,0.1)] bg-white shadow-sm">
                ...
                <p className="border-t border-[rgba(18,36,79,0.06)] bg-slate-50/80 px-5 py-3 text-[0.8rem] text-brand-muted leading-relaxed">
                  Short-term loans are structured as a single bullet repayment on the maturity date. Please complete payment through your registered bank mandate or matching dynamic UPI instructions sent to your registered contact number.
                </p>
              </div>
            </section>
          )}
        ...
        /* History / Previous Loans Tab Panel */
        <div className="grid gap-6 animate-fade-in-up">
          <section className="grid gap-4">
            <h2 className="text-lg font-extrabold text-brand-navy">Previous Loans & Applications</h2>
            {dash.pastLoans.length === 0 ? (
              <EmptyStateCard
                icon={<HistoryIcon />}
                title="No loan history yet"
                description="Closed, matured, or past loan applications will appear here to maintain a record of your financial history."
              />
            ) : (
