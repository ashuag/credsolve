...
import { runTenacioFaceLivenessCheck, type TenacioFaceCheckResult } from '@/lib/api/tenacio-face-checks';
...
export function TenacioFaceLivenessCheckPanel() {
  ...
  async function handleRunCheck() {
    ...
    if (!ready) {
      setError('Provide a photo, a captured selfie, or a link.');
      return;
    }
    ...
    setLoading(true);
    try {
      const out = await runTenacioFaceLivenessCheck(token, {
      ...
      setError(err instanceof Error ? err.message : 'Tenacio face liveness check failed.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="grid gap-5">
      <p className="m-0 text-[0.88rem] leading-[1.55] text-brand-muted">
        Live Tenacio face-liveness check, calling <code className="font-mono text-[0.8rem]">TENACIO_LIVENESS_*</code>{' '}
        directly. Tenacio fetches the selfie itself from a public URL — an uploaded photo or captured selfie is
        first pushed to S3 and resolved to a URL before the call. The call is audited in{' '}
      ...
      <div className="grid gap-3 rounded-[14px] border border-[rgba(23,44,113,0.1)] bg-white p-4 max-w-md">
        <ModeTabs
          mode={mode}
          disabled={loading}
          onChange={(next) => {
        ...
        {mode === 'link' ? (
          <label className="grid gap-1.5">
            ...
            <input
              ...
              disabled={loading}
              placeholder="https://… publicly reachable selfie photo"
        ...
        {preview ? (
          ...
          <div className="flex h-36 items-center justify-center rounded-[10px] border border-dashed border-[rgba(23,44,113,0.14)] bg-[rgba(248,250,255,0.65)] text-[0.78rem] text-brand-muted">
            {mode === 'link' ? 'Enter a public image URL' : 'No photo selected'}
          </div>
        ) : null}
      ...
      <label className="flex w-fit items-center gap-2 text-[0.84rem] font-bold text-brand-navy">
        ...
        Customer consent for the liveness check
      </label>

      <button
        type="button"
        disabled={loading || !ready || !consent}
      ...
      >
        {loading ? 'Checking…' : 'Run Tenacio liveness check'}
      </button>
