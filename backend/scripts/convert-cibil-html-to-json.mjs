 ...
 *
 * Usage:
 *   node scripts/convert-cibil-html-to-json.mjs [--out dir] [html-file-or-dir …]
 */
...
function buildVendorJson(htmlPath, html) {
  ...
  const borrowerKey = String(syntheticSerial(personal.name ?? htmlPath)).slice(0, 9);
  const referenceKey = String(syntheticSerial(`${htmlPath}|ref`)).slice(0, 11);
  ...
  return {
        ...
        GetCustomerAssetsResponse: {
          ...
          GetCustomerAssetsSuccess: {
            Asset: {
              ...
              Status: 'Active',
              AssetId: String(syntheticSerial(htmlPath)).slice(0, 11),
    ...
    _meta: {
      convertedFromHtml: basename(htmlPath),
...
function collectHtmlPaths(args) {
  const paths = [];
  for (const arg of args) {
    ...
    if (stat.isDirectory()) {
      for (const entry of readdirSync(resolved)) {
        if (entry.toLowerCase().endsWith('.html')) paths.push(join(resolved, entry));
      }
...
function main() {
  ...
  let outDir = resolve('storage/cibil_samples');
  const positional = [];
  ...
  const defaultInputs = [
    '/home/saurabha/Downloads/30+ last 3 months 1.html',
    '/home/saurabha/Downloads/30+ last 3 months 2.html',
    '/home/saurabha/Downloads/60+ last 9 months 1.html',
    '/home/saurabha/Downloads/60+ last 9 months 2.html',
    '/home/saurabha/Downloads/90+ last 12 months 1.html',
    '/home/saurabha/Downloads/90+ last 12 months 2.html',
    '/home/saurabha/Downloads/Pwos 1.html',
    '/home/saurabha/Downloads/Pwos 2.html',
    '/home/saurabha/Downloads/SMA (1).html',
    '/home/saurabha/Downloads/No wo, doubtful_settled in last 18 months 1.html',
    '/home/saurabha/Downloads/No wo, doubtful_settled in last 18 months 2.html',
  ];

  const htmlPaths = collectHtmlPaths(positional.length > 0 ? positional : defaultInputs);
  ...
  for (const htmlPath of htmlPaths) {
    ...
    const json = buildVendorJson(htmlPath, html);
    const outPath = join(outDir, `${slugify(htmlPath)}.json`);
    writeFileSync(outPath, JSON.stringify(json, null, 2));
    console.log(
      `Wrote ${outPath} (${json._meta.accountCount} accounts, score ${json.data.cibilData.GetCustomerAssetsResponse.GetCustomerAssetsSuccess.Asset.TrueLinkCreditReport.Borrower.CreditScore.riskScore})`,
    );
  }
}
