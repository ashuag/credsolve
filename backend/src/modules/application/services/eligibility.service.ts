...
import { type BureauReport, NsdlBureauService } from './nsdl-bureau.service';
...
function buildMockReport(): BureauReport {
...
export class EligibilityService {
  constructor(
    private readonly nsdlBureauService: NsdlBureauService,
  ...
  async check(params: {
    ...
    const get = (key: string, fallback: string) => criteria.find(c => c.key === key)?.value ?? fallback;
    ...
    const fail = (reason: string): EligibilityResult => ({
      isEligible: false,
