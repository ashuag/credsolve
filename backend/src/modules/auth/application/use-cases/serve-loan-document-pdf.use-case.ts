...
export class ServeLoanDocumentPdfUseCase {
  ...
  async execute(req: Request, res: Response, docTypeRaw: string): Promise<void> {
    ...
    // PDFs are generated by LOS, not on-demand in the customer journey.
    const rel = this.loanDocs.relativePathForType(docType, app)?.trim() || null;
    if (!rel) {
      throw new ServiceUnavailableException('This document has not been generated yet. Please contact support.');
    }

    if (!(await this.kycFiles.exists(rel))) {
      throw new ServiceUnavailableException('Document file is unavailable. Please contact support.');
    }
