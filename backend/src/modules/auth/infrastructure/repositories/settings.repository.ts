...
export class SettingsRepository implements OnModuleInit {
  ...
  /** Min CIBIL thresholds from `eligibility_criteria`. */
  async loadCibilEligibilityThresholds(): Promise<CibilEligibilityThresholds> {
    const keys = [EC.CIBIL_MIN_NEW, EC.CIBIL_MIN_EXISTING];
    ...
    return {
      cibilMinNew: pick(EC.CIBIL_MIN_NEW, 700),
      cibilMinExisting: pick(EC.CIBIL_MIN_EXISTING, 650),
    };
  }
  ...
  async loadBreSettings(): Promise<BreSettings> {
    const criteriaKeys = [
      ...
      EC.MAX_AGE,
      EC.REJECTED_GENDERS,
      EC.REJECTED_OCCUPATIONS,
    ];

    const criteriaRows = await this.prisma.client.eligibilityCriteria.findMany({
