  ...
  async getCustomerDetails(customerUuid: string) {
    ...
    return {
      ...
      leads: customer.leads.map((lead) => ({
        ...
        panVerified: lead.leadDetail?.panVerified ?? 0,
        panVerifiedLabel: panVerifiedStatusLabel(lead.leadDetail?.panVerified ?? 0),
        bureauFetched: lead.leadDetail?.bureauFetched ?? 0,
