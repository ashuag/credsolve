...
export class LivenessVendorService {
  ...
  async postLivenessCheck(
    ...
    this.logger.log(
      storagePath
        ? `Tenacio liveness request (selfiePath=${storagePath}): ${JSON.stringify(livenessInput)}`
        : `Tenacio liveness request: ${JSON.stringify(livenessInput)}`,
    );
    ...
    this.logger.log(
      `Tenacio liveness response: HTTP ${result.httpStatus ?? 'n/a'} body=${JSON.stringify(result.body ?? result.rawText ?? null)}`,
    );
